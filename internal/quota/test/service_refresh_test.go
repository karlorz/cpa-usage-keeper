...
func TestRefreshAcceptsActivePoeAIProviderIdentity(t *testing.T) {
	db := openQuotaTestDatabase(t)
	...
	response, err := service.Refresh(context.Background(), RefreshRequest{AuthIndexes: []string{"poe-auth"}, Source: RefreshSourceManual})
	if err != nil {
		t.Fatalf("Refresh returned error: %v", err)
	}
	if response.Accepted != 1 || len(response.Tasks) != 1 || response.Tasks[0].AuthIndex != "poe-auth" {
		t.Fatalf("unexpected refresh response: %+v", response)
	}
	waitForRefreshTask(t, service, "poe-auth", RefreshTaskStatusCompleted)
	service.WaitRefreshTasks()
	if handler.callCount() != 1 {
		t.Fatalf("expected poe AI provider to refresh once, got %d calls", handler.callCount())
	}
}
